...
const ORCHESTRATOR_RELATIVE_PATH = path.join('.agents-workspace', 'orchestrator.md');

export function readOrchestrator(worktree: string): string | undefined {
  try {
    return readFileSync(path.join(worktree, ORCHESTRATOR_RELATIVE_PATH), 'utf-8').trim();
  } catch {
    return undefined;
  }
}

export function writeOrchestrator(worktree: string, orchestrator?: string): void {
  if (!orchestrator) {
    // Remove persisted orchestrator when not provided.
    try {
      unlinkSync(path.join(worktree, ORCHESTRATOR_RELATIVE_PATH));
    } catch {
  ...
  mkdirSync(path.join(worktree, '.agents-workspace'), { recursive: true });
  writeFileSync(path.join(worktree, ORCHESTRATOR_RELATIVE_PATH), orchestrator + '\n', 'utf-8');
}
...
/**
 * Submit tasks for inclusion in the orchestrator plan.
 *
 * `agents` is the full roster of valid agent slugs for this session.
 * `orchestrator` is the slug of the agent acting as orchestrator, if any.
 *
 * The caller (the orchestrator agent) is responsible for:
 *  - mirroring accepted tasks to the host `todowrite` board after this
 *    function returns ok:true;
 *  - dispatching ready tasks via the standard `task` tool.
 */
async function submitTasks(
  ...
  const validation = validateTasks(tasks, existing, { agents, orchestrator });
  if (!validation.ok) {
    return { ...validation };
  }
  ...
  writeOrchestrator(worktree, orchestrator);
    ...
    tool: {
      submit_tasks: tool({
        ...
        args: {
          tasks: tool.schema
            .array(
              tool.schema.object({
                ...
                assignedAgent: tool.schema.string().describe('Agent slug'),
          ...
          agents: tool.schema.array(tool.schema.string()).describe('The roles participating in this session (used to validate task assignments). Pass an empty array to disable role enforcement.'),
          orchestrator: tool.schema.string().optional().describe('The slug of the orchestrator agent submitting these tasks.'),
        },
        async execute(args, _ctx) {
          const result = await submitTasks(worktree, args.tasks as TaskRecord[], args.agents as string[], args.orchestrator);
          if (!result.ok) {
