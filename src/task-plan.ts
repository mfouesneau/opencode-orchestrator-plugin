 ...
 *  - The orchestrator cannot submit implementation work targeted at itself —
 *    it routes implementation to a specialist agent instead.
 */
export function validateTasks(
  ...
  // Normalize agent names to lowercase for comparison.
  const agentsLower = new Set((config.agents ?? []).map((a) => a.toLowerCase()));
  const hasRoster = Array.isArray(config.agents) && config.agents.length > 0;
  ...
  // 2. Agent valid + orchestrator constraint
  const orchestratorLower = (config.orchestrator ?? '').toLowerCase();
  const submittingAgentLower = (config.submittingAgent ?? '').toLowerCase();
  for (const t of tasks) {
    ...
    if (hasRoster) {
      if (!agentsLower.has(agent)) {
        errors.push(
          `task "${t.id}" has unknown agent "${t.assignedAgent}" (must be one of: ${[...agentsLower].join(', ')})`,
        );
      }
      if (
        submittingAgentLower &&
        orchestratorLower &&
        submittingAgentLower === orchestratorLower &&
        agent === orchestratorLower &&
        looksLikeImplementation(t.prompt)
      ) {
        errors.push(
          `task "${t.id}" cannot be submitted: orchestrator "${orchestratorLower}" must not do implementation work. ` +
            `Route this prompt to a specialist agent and re-submit.`,
        );
      }
    }
  }
