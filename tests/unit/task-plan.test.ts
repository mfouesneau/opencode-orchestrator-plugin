...
describe('validateTasks', () => {
  test('T1: well-formed task is accepted', () => {
    const result = validateTasks([makeTask('a')], [], { agents: ['alpha', 'beta', 'gamma'] });
  ...
  test('T2b: ID colliding with existing plan is rejected', () => {
    ...
    const result = validateTasks([makeTask('existing-id')], existing, { agents: ['alpha', 'beta', 'gamma'] });
    assert.strictEqual(result.ok, false);
    if (!result.ok) {
  ...
  test('T3: unknown assignedAgent is rejected', () => {
    const result = validateTasks([makeTask('a', { assignedAgent: 'nobody' })], [], { agents: ['alpha', 'beta', 'gamma'] });
    assert.strictEqual(result.ok, false);
    if (!result.ok) {
      assert.ok(
  ...
  test('T3b: agent validation is case-insensitive', () => {
    const result = validateTasks([makeTask('a', { assignedAgent: 'BETA' })], [], { agents: ['alpha', 'beta', 'gamma'] });
    assert.deepEqual(result, { ok: true }, 'case-insensitive agent matching works');
  });

  test('T3c: empty assignedAgent is rejected', () => {
    const result = validateTasks([makeTask('a', { assignedAgent: '' })], [], { agents: ['alpha', 'beta', 'gamma'] });
    assert.strictEqual(result.ok, false);
  });

  test('T4: dependsOn referencing an unknown ID (not in batch nor in existing plan) is rejected', () => {
    const result = validateTasks([makeTask('a', { dependsOn: ['ghost'] })], [], { agents: ['alpha', 'beta', 'gamma'] });
    assert.strictEqual(result.ok, false);
    if (!result.ok) {
  ...
  test('T4b: dependsOn referencing an existing-plan task is accepted', () => {
    ...
    const result = validateTasks([makeTask('child', { dependsOn: ['parent'] })], existing, { agents: ['alpha', 'beta', 'gamma'] });
    assert.deepEqual(result, { ok: true });
  });

  test('T5a: self-dependency is detected as a cycle', () => {
    const result = validateTasks([makeTask('a', { dependsOn: ['a'] })], [], { agents: ['alpha', 'beta', 'gamma'] });
    assert.strictEqual(result.ok, false);
    if (!result.ok) {
      assert.ok(
  ...
  test('T6a: orchestrator submitting an implementation task targeted at itself is rejected', () => {
    const result = validateTasks(
      [makeTask('a', { assignedAgent: 'alpha', prompt: 'Implement the auth module' })],
      [],
      { agents: ['alpha', 'beta'], orchestrator: 'alpha', submittingAgent: 'alpha' },
    );
    assert.strictEqual(result.ok, false, 'orchestrator self-implementation must be rejected');
    if (!result.ok) {
      assert.ok(
  ...
  test('T6b: orchestrator doing planning for itself is allowed', () => {
    const result = validateTasks(
      [makeTask('a', { assignedAgent: 'alpha', prompt: 'Plan the rollout sequence' })],
      [],
      { agents: ['alpha', 'beta'], orchestrator: 'alpha', submittingAgent: 'alpha' },
    );
    assert.deepEqual(result, { ok: true }, 'orchestrator may plan/dispatch when not implementing');
  });

  test('T6c: orchestrator routing implementation to another agent is allowed', () => {
    const result = validateTasks(
      [makeTask('a', { assignedAgent: 'beta', prompt: 'Implement the auth module' })],
      [],
      { agents: ['alpha', 'beta'], orchestrator: 'alpha', submittingAgent: 'alpha' },
    );
    assert.deepEqual(result, { ok: true }, 'orchestrator may route implementation work to a specialist');
  });

  test('T6d: non-orchestrator submitter is not subject to the agent-role check', () => {
    const result = validateTasks(
      [makeTask('a', { assignedAgent: 'alpha', prompt: 'Implement the auth module' })],
      [],
      { agents: ['alpha', 'beta'], orchestrator: 'alpha', submittingAgent: 'beta' },
    );
    assert.deepEqual(result, { ok: true }, 'non-orchestrator submitters bypass the check');
  });

  test('T6e: non-orchestrator submitter bypasses check even when target is the orchestrator', () => {
    const result = validateTasks(
      [makeTask('a', { assignedAgent: 'alpha', prompt: 'Implement the auth module' })],
      [],
      { agents: ['alpha', 'beta'], orchestrator: 'alpha', submittingAgent: 'beta' },
    );
    assert.deepEqual(result, { ok: true }, 'non-orchestrator submitters bypass the check regardless of target');
  });

  test('unknown agent rejected when agents[] is non-empty', () => {
    const result = validateTasks(
      [makeTask('a', { assignedAgent: 'wizard' })],
      [],
      { agents: ['king', 'queen'] },
    );
    assert.equal(result.ok, false);
    assert.ok((result as { ok: false; errors: string[] }).errors.some((e) => e.includes('wizard')));
  });

  test('empty agents[] skips agent validation but still checks deps and cycles', () => {
    const result = validateTasks(
      [makeTask('a')],
      [],
      { agents: [] },
  ...
  test('orchestrator self-implementation is rejected only when submittingAgent === orchestrator', () => {
    // submittingAgent matches orchestrator → rejected
    const r1 = validateTasks(
      [makeTask('a', { assignedAgent: 'alpha', prompt: 'Implement the auth module' })],
      [],
      { agents: ['alpha', 'beta'], orchestrator: 'alpha', submittingAgent: 'alpha' },
    );
    ...
    // submittingAgent differs from orchestrator → allowed even though target === orchestrator
    const r2 = validateTasks(
      [makeTask('a', { assignedAgent: 'alpha', prompt: 'Implement the auth module' })],
      [],
      { agents: ['alpha', 'beta'], orchestrator: 'alpha', submittingAgent: 'beta' },
    );
...
describe('plugin submit_tasks rejects invalid input', () => {
  ...
  test('returns ok and persists tasks when valid', async () => {
    ...
    const worktree = freshWorktree();
    try {
      const result = await __testing__.submitTasks(worktree, [makeTask('a')], ['alpha', 'beta']);
      ...
      assert.strictEqual(persisted[0].assignedAgent, 'alpha');
    } finally {
      cleanup(worktree);
    }
  });
  ...
  test('completeTask marks a task done and persists the result', async () => {
    ...
    const worktree = freshWorktree();
    try {
      await __testing__.submitTasks(worktree, [makeTask('a')], ['alpha', 'beta']);
  ...
  test('taskStatus returns a markdown table', async () => {
    ...
    const worktree = freshWorktree();
    try {
      await __testing__.submitTasks(worktree, [makeTask('a'), makeTask('b')], ['alpha', 'beta']);
